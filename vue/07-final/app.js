...
var app1 = new Vue({
    ...
    data: {
        ...
        longitude: '',
        onUpdateId: -1,
        markerOnCreate: false,
        markerOnEdit: false,
        markers: {},
    },
    methods: {
        changeMarkerOnCreate: function () {
	  this.markerOnCreate = !this.markerOnCreate
	},
	changeMarkerOnEdit: function () {
	  this.markerOnEdit = !this.markerOnEdit
	},
	...
	createMapEdit: function (id, latitude,longitude) {
	  ...
	  this['map_'+id].on('click', (e) => {
	    if(this.markerOnEdit && this.onUpdateId != -1) {
        ...
        updateHospital: function (hospital)   {
        	...
        	if(this.markers[hospital.id]) {
        		this.map_to_all.removeLayer(this.markers[hospital.id]);
        	}

                let latlng = {lat: Number(hospital.latitude), lng: Number(hospital.longitude)}
	        let m = new L.marker(latlng).addTo(this.map_to_all)
    ...
    mounted () {
 	...
 	this.map_to_all = L.map('map_to_all').setView([-22.905, -43.190], 13);
		...
		attribution: 'Map data &copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors, ' +
			...
			'Imagery © <a href="https://www.mapbox.com/">Mapbox</a>',
		id: 'mapbox.streets'
	}).addTo(this.map_to_all);

        this.map_to_all.on('click', (e) => {
            //var marker = new L.marker(e.latlng).addTo(map);
	            this.latitudeSearch = e.latlng.lat
	            this.longitudeSearch = e.latlng.lng
	            console.log('Maker: ',e);
		        if(this.markerSearch) {
		          this.map_to_all.removeLayer(this.markerSearch);
		        }
		    // para dentro do marker voce passa um objeto {lat, lng}
		        this.markerSearch = new L.marker(e.latlng).addTo(this.map_to_all);
		        this.hospitais.forEach((hospital) => {
	    	        if(this.markers[hospital.id]) {
	...
	var map_to_create = L.map('map_to_create').setView([-22.905, -43.190], 13);
	...
	map_to_create.on('click', (e) => {
            //var marker = new L.marker(e.latlng).addTo(map);
	    console.log('Maker: ',this.markerOnCreate);
	    if(this.markerOnCreate) {
	        this.latitude = e.latlng.lat
	        this.longitude = e.latlng.lng
	        console.log('Maker: ',e);
		    if(this.markerCreate) {
		      map_to_create.removeLayer(this.markerCreate);
		    }
		    // para dentro do marker voce passa um objeto {lat, lng}
		    this.markerCreate = new L.marker(e.latlng).addTo(map_to_create);
	    }
